...
- [ ] A4. Deploy: push `dist/` to Vercel (framework preset "Other" / static) or Netlify. Add a simple deploy script or CI (`eas build`/GH Action) that runs typecheck → export → deploy.
